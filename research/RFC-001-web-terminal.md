...
## Contexte
Actuellement hack3270 necessite un emulateur TN3270 externe (x3270) pour interagir avec le mainframe. La Web UI affiche les outils d'audit mais pas l'ecran 3270 lui-meme. L'utilisateur doit jongler entre deux fenetres.

## Proposition
Ajouter un onglet "Terminal" dans la Web UI qui rend l'ecran 3270 directement dans le navigateur. Le proxy hack3270 devient a la fois l'outil d'audit ET l'emulateur.
...
- **Negotiation telnet** : actuellement c'est x3270 qui negocie le handshake TN3270/TN3270E avec le serveur. Sans x3270, hack3270 devra gerer la negotiation telnet lui-meme (terminal-type, binary mode, EOR).
...
web.py (nouveau endpoint /api/screen + /api/input)
    |
    v
libhack3270.py (buffer ecran 80x24 + envoi donnees serveur)
    |  socket direct
    v
Mainframe (DVCA)
```
...
1. **libhack3270.py** : maintenir un buffer ecran 80x24 caracteres + attributs, mis a jour par parse_screen_map(). Ajouter methode pour construire les paquets client (AID + cursor + field data).
...
4. **hack3270.py** : mode sans client_connect() quand terminal web actif.
